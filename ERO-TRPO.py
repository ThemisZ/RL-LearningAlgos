...
class PolicyNet(torch.nn.Module):
    ...
    def forward(self, x):
        x = F.relu(self.fc1(x))
        return F.softmax(self.fc2(x), dim=1)  # 返回概率分布
...
class TRPO:
    """ TRPO算法 - ERO-TRPO版本（加入熵正则化）"""
    def __init__(self, state_dim, hidden_dim, action_dim, lmbda,
                 kl_constraint, alpha, critic_lr, gamma, device, entropy_coef=0.01):
        # 策略网络参数不需要优化器更新
        ...
        self.entropy_coef = entropy_coef  # 熵正则化系数（α）
        self.device = device
    ...
    def compute_surrogate_obj(self, states, actions, advantage, old_log_probs, actor):  
        ...
        entropy = action_dists.entropy().mean()  # 平均熵
        
        # 代理目标 = E[ratio * A] + α * E[H(π)]
        return torch.mean(ratio * advantage) + self.entropy_coef * entropy
